...
;(function (exports) {
  ...
  exports.convert = {
    ...
    'two args - first is dir w/ pngout': function (test) {
      ...
      svg_to_png
        .convert(
          path.resolve(path.join(__dirname, 'files')),
          path.resolve(path.join(__dirname, 'output', 'png'))
        )
        .then(function () {
          test.ok(
            fs.existsSync(path.join(__dirname, 'output', 'png', 'bear.png'))
          )
          test.done()
        })
    }
  ...
  exports.checkOutputSize = {
    ...
    tearDown: function (done) {
      if (
        fs.existsSync(
          path.join(__dirname, 'output', 'Internet-44px-width-44px-height.png')
        )
      ) {
        fs.unlinkSync(
          path.join(__dirname, 'output', 'Internet-44px-width-44px-height.png')
      ...
      if (
        fs.existsSync(
          path.join(__dirname, 'output', 'Internet-no-width-no-height.png')
        )
      ) {
        fs.unlinkSync(
          path.join(__dirname, 'output', 'Internet-no-width-no-height.png')
      ...
      if (
        fs.existsSync(
          path.join(
            __dirname,
            'output',
            'Internet-100pct-width-100pct-height.png'
          )
        )
      ) {
        fs.unlinkSync(
      ...
      if (
        fs.existsSync(
          path.join(
            __dirname,
            'output',
            'Internet-100pct-width-100pct-height-no-viewbox.png'
          )
        )
      ) {
        fs.unlinkSync(
      ...
      svg_to_png
        .convert(
          path.join(
            __dirname,
            'viewbox',
            'Internet-44px-width-44px-height.svg'
          ),
        ...
        .then(function () {
          sizeOf(
            path.join(
              __dirname,
              'output',
              'Internet-44px-width-44px-height.png'
            ),
    ...
    'no width/height, and viewbox': function (test) {
      ...
      svg_to_png
        .convert(
          path.join(__dirname, 'viewbox', 'Internet-no-width-no-height.svg'),
          path.join(__dirname, 'output')
        )
        .then(function () {
          sizeOf(
      ...
      svg_to_png
        .convert(
          path.join(
            __dirname,
            'viewbox',
            'Internet-100pct-width-100pct-height.svg'
          ),
          path.join(__dirname, 'output')
        )
        .then(function () {
      ...
      svg_to_png
        .convert(
          path.join(
            __dirname,
            'viewbox',
            'Internet-100pct-width-100pct-height-no-viewbox.svg'
          ),
          path.join(__dirname, 'output')
        )
        .then(function () {
  ...
  exports.convertWithDir = {
    ...
    "two args - first is file that doesn't exist": function (test) {
      ...
      test.throws(function () {
        svg_to_png.convert(
          path.resolve(path.join(__dirname, 'files', 'foo.svg')),
          path.resolve(path.join(__dirname, 'output'))
        )
      })
      test.done()
    ...
    'two args - first is file': function (test) {
      ...
      svg_to_png
        .convert(
          path.join(__dirname, 'files', 'bear.svg'),
          path.join(__dirname, 'output')
        )
        .then(function () {
          test.ok(fs.existsSync(path.join(__dirname, 'output', 'bear.png')))
  ...
  exports.convertAndCompressLarge = {
    ...
    tearDown: function (done) {
      var outputFiles = fs
        .readdirSync(path.resolve(path.join(__dirname, 'output')))
        .map(function (file) {
    ...
    'two args - first is arr': function (test) {
      ...
      // tests here
      var bigDir = fs
          .readdirSync(path.resolve(path.join(__dirname, 'big')))
          .map(function (file) {
      ...
      svg_to_png
        .convert(bigDirSVGs, path.join(__dirname, 'output'), {
          compress: true
        })
        .then(function () {
          test.ok(true)
