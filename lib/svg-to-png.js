...
exports.convert = function (input, output, opts) {
  ...
  if (
    ...
    throw new Error(
      'Input must be Array of files or String that is a directory'
    )
  }
  ...
  if (
    ...
    throw new Error(
      'All files passed into svg-to-png must be SVG files, that includes all files in a directory'
    )
  }

  if (!files.length) {
    throw new Error(
      'Input must be Array of SVG files or String that is a directory that contains some of those'
    )
  }
